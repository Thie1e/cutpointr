...
1.  Determine optimal cutpoints with bootstrapping (as an alternative to
    determine them without bootstrapping)
...
-   `$optimal_cutpoint`: The optimal cutpoint, which was determined by
...
-   `$specificity`: The specificity when applying the cutpoint to the full
    dataset.
-   `$youden`: The maximal Youden-Index (= sensitivity + specificity - 1), which
    was determined by the optimization.
...
-   `boot_stratify`: If the bootstrap samples are drawn in both classes
    separately before combining them to keep the number of positives/negatives
...
-   `$optimal_cutpoint`: The optimal cutpoint, which is the aggregated value (as
    ...
    note that no uncertainty measure (standard deviation, 95%-CI, etc.) are
    available here.
...
subsequently with bootstrapping. This can easily be activated by setting the
...
However, the method to calculate the optimal cutpoints will then be performed
subsequently on n=`boot_runs` bootstrap samples. For each of these bootstrap
...
In the validation routine, the chosen cutpoint optimization then repeated in
