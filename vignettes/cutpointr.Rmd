...
chosen using the `cutpoint_knots` function. That function implements a rule of 
thumb for selecting the number of knots as $\lceil 0.1 * log(n / n_{cut}) * n_{cut} \rceil + 1$
where $n$ is the number of observations and $n_{cut}$ is the number of unique
cutpoints found in the data.

Alternatively, the number of knots
can be set manually or the other smoothing parameters of `stats::smooth.spline`
